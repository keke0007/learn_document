...
Promise.allsettled() 返回一个 promise 对象， 状态和PromiseResult的规则如下：

```
1. 参数是可遍历对象，返回值是个成功状态的 promise 对象，所有的参数的成员都完成，返回值状态才改变， PromiseResult是一个数组，数组中每个成员与参数每个成员按照顺序对应 
2. 数组的成员对应的成功 {status value}, 如果对应的是失败的 {status,reason}
3. 如果参数不是可遍历对象，返回失败状态的 promise 对象      
```
